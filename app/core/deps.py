...
async def require_proxy_secret(x_rapidapi_proxy_secret: str = Header(None)):
    """When RAPIDAPI_PROXY_SECRET is set, only allow RapidAPI-proxied requests — this
    locks the origin so the (powerful) renderer can't be hit directly/unauthenticated.
    When unset (local/dev), requests pass through."""
